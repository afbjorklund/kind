...
// nodes.Node implementation for the singularity provider
type node struct {
	name string
}
...
func (n *node) IP() (ipv4 string, ipv6 string, err error) {
	// retrieve the IP address of the node using singularity exec
...
// nodeCmd implements exec.Cmd for singularity nodes
type nodeCmd struct {
...
func (c *nodeCmd) Run() error {
	...
	// args the command in the container rather than to singularity
	args = append(
		args,
	...
	var cmd exec.Cmd
	if c.ctx != nil {
		cmd = exec.CommandContext(c.ctx, "singularity", args...)
	} else {
